...
class DatafitSupabaseUser extends BaseAuthUser {
  DatafitSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> datafitSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DatafitSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
